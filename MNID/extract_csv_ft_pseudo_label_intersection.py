...
def get_melody_events(events, inputs, raw_information, preds, raw_preds, pad_CP, bridge=True):
    ...
    for event in events:
        ...
        if len(event) == 5:     # filter out melody events
            ...
            if is_valid_note and is_melody:
                ...
                melody_raw_information.append(list(raw_information[total_notes-1]))
                # melody_raw_information[-1][-1] = 'a'
            elif is_valid_note:
                melody_raw_information.append(list(raw_information[total_notes-1]))
                melody_raw_information[-1][-1] = ''

            # print (melody_raw_information[-1])
    ...
    # print (len(melody_raw_information), len(raw_information))
...
def output_csv_pseudo_label(cleaned_note_list, target_csv_path):
...
def main():
    ...
    folds_lookup = [i for i in range(35)]
    ...
    for fold in range(5):
        ...
        for cur_fold_id in test_fold:
            for i in range(len(folds_lookup)):
                test_indices.append(folds_lookup[i] + 1)
        ...
        for i in test_indices:
            piece = str(i).zfill(2)
            cur_input_path = os.path.join('../mozart_pseudo_melody', piece + '-1.csv')
            # cur_input_path = os.path.join('../mozart_sonata_clean_skyline', piece + '-1.csv')
            cur_db_path = os.path.join('../mozart_sonata_clean_correct_dbeat', piece + '-1.csv')
            ...
            os.makedirs('../mozart_pseudo_melody_1105_' + str(fold), exist_ok=True)
            output_path = os.path.join('../mozart_pseudo_melody_1105_' + str(fold), piece + '-1.csv')
            # os.makedirs('../mozart_sonata_clean_correct_skyline_1105_' + str(fold), exist_ok=True)
            # output_path = os.path.join('../mozart_sonata_clean_correct_skyline_1105_' + str(fold), piece + '-1.csv')
            # post-process    
            ...
            # print (len(raw_information), len(melody_raw_information))
            # for i in range(len(raw_information)):
            #     print (raw_information[i][-1], melody_raw_information[i][-1])
            test_mean_results += np.array(test_results)
            output_csv_pseudo_label(melody_raw_information, output_path)
