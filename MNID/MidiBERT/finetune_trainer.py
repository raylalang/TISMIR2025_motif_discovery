    ...
    def iteration(self, training_data, mode, seq):
        ...
        for x, y in pbar:  # (batch, 512, 768)
            ...
            if not seq:
                ...
                # print (y)
                # print (output)
                confusion[0][0] += int(torch.sum(((y == 3) & (output != 1)).float() * attn))
                confusion[0][1] += int(torch.sum(((y == 3) & (output == 1)).float() * attn))
                confusion[1][0] += int(torch.sum(((y == 1) & (output != 1)).float() * attn))
                confusion[1][1] += int(torch.sum(((y == 1) & (output == 1)).float() * attn))
            else:
