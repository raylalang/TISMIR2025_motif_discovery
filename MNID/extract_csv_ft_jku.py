...
def get_args():
    ...
    ### path setup ###
    parser.add_argument('--input_dir', type=str, required=True, help="directory to the input csv files")
    parser.add_argument('--db_dir', type=str, required=True, help="directory to the downbeat files")
    parser.add_argument('--output_csv_dir', type=str, default=None, help="directory to save the output csv files")
...
def get_melody_events(events, inputs, raw_information, preds, raw_preds, pad_CP, bridge=True):
    ...
    for event in events:
        if len(event) == 5:     # filter out melody events
            ...
            # print (is_melody,  raw_information[total_notes][-1])
            if is_valid_note:
    ...
    print (test_f1, test_accuracy, test_precision, test_recall
        , confusion[1][0] + confusion[1][1], np.sum(confusion), confusion)
...
def main():
    ...
    for fold in range(5):
        ...
        model.append(load_model(args, fold, e2w, w2e).to(device))
        model[fold].eval()

    for i in range(len(test_data_list)):
        test_indices.append(test_data_list[i] + 1)
    ...
    for i in test_indices:
        ...
        cur_db_path = os.path.join(args.db_dir, f'{piece}-1.csv')
        ...
        with torch.no_grad():
            ...
            for i in range(len(model)):
                predictions, cur_raw_preds = inference(model[i], tokens, pad_CP, device)
                raw_preds = raw_preds + cur_raw_preds

                predictions2, cur_raw_preds = inference(model[i], tokens2, pad_CP, device)
                raw_preds2 = raw_preds2 + cur_raw_preds

                predictions3, cur_raw_preds = inference(model[i], tokens3, pad_CP, device)
                raw_preds3 = raw_preds3 + cur_raw_preds
        ...
        test_5fold_mean_results_list.append(test_results)
    ...
    print ('Test mean F1: {:.3f} Acc: {:.3f} P: {:.3f} R: {:.3f}'.format(test_mean_results[0]
    ...
    print ('Std for 5-fold F1: {:.3f} Acc: {:.3f} P: {:.3f} R: {:.3f}'.format(test_5fold_std[0]
