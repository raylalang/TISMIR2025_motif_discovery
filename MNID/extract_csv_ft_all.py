...
def get_args():
    ...
    parser.add_argument('--db_path', type=str, required=True, help="path to the downbeat file")
    ...
    if not args.output_path:
        ...
        args.output_path = os.path.join(args.output_dir, f'{basename}_melody.mid')
...
def inference(model, tokens, pad_CP, device):
    ...
    return torch.from_numpy(output), pred
...
def main():
    ...
    # output_clean_csv_dir = '../Beethoven_midibert_ft'
    output_clean_csv_dir = args.output_csv_dir
    ...
    #         piece = str(i).zfill(2)
    #         cur_input_path = args.input_path.replace('01', piece)
    #         cur_db_path = args.db_path.replace('1', str(i))

    #         if i == 8:
    #             cur_db_path = cur_db_path.replace('dBeats', 'dbeats')
    #         events, tokens, raw_information = CP_model.prepare_data_from_csv(cur_input_path, cur_db_path, args.max_seq_len)

    #         # events, tokens = CP_model.prepare_data_from_csv(args.input_path, args.db_path, args.max_seq_len)

    #         # filename = args.input_path.split('/')[-1]
    #         filename = cur_input_path.split('/')[-1]
    ...
    #         output_path = args.output_path.replace('01', piece)
    #         # _ = events2midi(events, args.output_path[:-11] + '_orig.mid')
    #         _ = events2midi(events, output_path[:-11] + '_orig.mid')
    #         # post-process    
    ...
    #         os.makedirs(output_clean_csv_dir, exist_ok=True)
    #         output_csv_to_file(melody_raw_information, os.path.join(output_clean_csv_dir, piece+'-1.csv'))

    #         # save melody midi
    #         # melody_midi = events2midi(melody_events, args.output_path)
    #         melody_midi = events2midi(melody_events, output_path)
    ...
    dataset_song_num = len(os.listdir(os.path.dirname(args.input_path)))
    print (dataset_song_num)
    folds_lookup = [[i for i in range(dataset_song_num)]]
    test_5fold_mean_results = np.array([0.0, 0.0, 0.0, 0.0])
    ...
    for fold in range(1):
        ...
        for i in test_indices:
            piece = str(i).zfill(2)
            cur_input_path = args.input_path.replace('01', piece)
            cur_db_path = args.db_path.replace('01-', piece+'-')
