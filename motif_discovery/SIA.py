...
def find_motives(notes, horizontalTolerance=0, verticalTolerance=3, adjacentTolerance=(2, 12),
    ...
    def check_adjacency(context, check_vertical=False):
        ...
        # if len(context) and context[0] == 48:
        #     print (context, ioi, ipi, adjacentTolerance[0])
        return context
    ...
    print('number of unique notes =', n_notes, '; len(context_dict.keys()) =', len(context_dict.keys()))
    clkStart = time.time()
    ...
    for i in sorted(context_dict.keys()):
        ...
        # if (len(cotext_pairs) and len(cotext_pairs[0][0]) and
        #     notes[cotext_pairs[0][0][0]]['onset'] >= 41.82 and notes[cotext_pairs[0][0][0]]['onset'] <= 41.84):
        #     print (cotext_pairs[11])
        #     print ([list(notes[cotext_pairs[11][0][i]][['onset', 'pitch']]) for i in range(len(cotext_pairs[11][0]))])
        #     print ([list(notes[cotext_pairs[11][1][i]][['onset', 'pitch']]) for i in range(len(cotext_pairs[11][1]))])
        cotext_pairs = [pair for pair in cotext_pairs if len(pair[0]) >= min_notes and len(pair[1]) >= min_notes]
        # if len(cotext_pairs) and notes[cotext_pairs[0][0][0]]['onset'] >= 41.82 and notes[cotext_pairs[0][0][0]]['onset'] <= 41.84:
        #     print (cotext_pairs)
        cotext_pairs = [pair for pair in cotext_pairs if melodic_check(pair[0]) and melodic_check(pair[1])]

        if len(cotext_pairs):
            # if notes[cotext_pairs[0][0][0]]['onset'] >= 41.82 and notes[cotext_pairs[0][0][0]]['onset'] <= 41.84:
            #     print (cotext_pairs)
            ...
            # Establish a pattern
            # print (cotext_dict.items())
            key, occurrences = max(cotext_dict.items(), key=lambda t: len(t[1]))
            # if notes[occurrences[0][0]]['onset'] >= 41.82 and notes[occurrences[0][0]]['onset'] <= 41.84:
            #     print ([list(notes[occur][['onset', 'pitch']]) for occur in occurrences])
    ...
    if len(pkeys) == 0:
        # dummy output
        pattern_list = [[[[-200.0, -100.0], [-195.0, -100.0]]]]
        return pattern_list
