...
def read_xlsx_rows(file_path, sheet_index=0):
    wb = openpyxl.load_workbook(file_path, read_only=True, data_only=True)
    ws = wb.worksheets[sheet_index]
    # yields each row as a list of values (None preserved)
...
def get_training_data(label_type=None):
    ...
    for i in range(32):
        ...
        dbeats = []
        for cols in read_xlsx_rows(fileDir, sheet_index=0):
            if not cols or cols[0] is None:
                continue
            try:
                dbeats.append(float(cols[0]))
            except (TypeError, ValueError):
                # header like "downbeat" etc.
                continue

        dbeat_labels[i] = dbeats
    ...
    for i in range(32):
        ...
        for cols in read_xlsx_rows(fileDir, sheet_index=0):
            ...
            # optional: skip header row if onset isn't numeric
            if not isinstance(cols[0], (int, float)):
                continue

            # degree column: if Excel stored it as float, turn into int-like string
            # (your dtype says degree is '<U10', so keep it as string)
            if isinstance(cols[3], float) and cols[3].is_integer():
            ...
            # ensure strings for string fields, int for inversion
            for j in [2, 4, 6]:
                cols[j] = "" if cols[j] is None else str(cols[j])

            cols[5] = 0 if cols[5] is None else int(cols[5])

            # onset/end must be floats
            cols[0] = float(cols[0])
            cols[1] = float(cols[1])
        ...
        chords = np.array(chords, dtype=t)
        chord_labels[i] = chords
        chord_timing_labels[i] = np.array(chord_timings, dtype=float)
    ...
    for i in range(32):
        fileDir = os.path.join(
            path,
            "../Beethoven_motif-main/csv_notes_clean",
            str(i + 1).zfill(2) + "-1.csv",
        )
        with open(fileDir, "r") as f:
            reader = csv.reader(f, delimiter=",")
            notes = []
    ...
    for i in range(32):
        ...
        for j in range(len(bps_notes[i])):
            ...
            if last_downbeat_id == 0:
                last_downbeat = dbeat_labels[i][0] - (
                    dbeat_labels[i][1] - dbeat_labels[i][0]
                )
            else:
            ...
            other_labels.append(
                [
                    cur_localbeat,
                    chord_labels[i][last_chordtime_id]["key"],
                    chord_labels[i][last_chordtime_id]["degree"],
                    chord_labels[i][last_chordtime_id]["quality"],
                    chord_labels[i][last_chordtime_id]["inversion"],
                    chord_labels[i][last_chordtime_id]["rchord"],
                    cur_motif_type,
                    boundary,
                    0,
                    0,
    ...
    for i in range(32):
        target_csv_path = os.path.join(
            "../cnn_v2_128/dataset_pianoroll/new_dest", str(i + 1).zfill(2) + "-1.csv"
        )
