...
def load_all_motives(filename_csv, filename_midi):
    ...
    for motif in motives_csv:
        ...
        for i in range(len(motif_notes['onset'])):
            motif_notes['onset'][i] = float(int(round(float(motif_notes['onset'][i]) * 100.0))) / 100
...
def run_one_data(csv_note_dir, csv_label_dir, motif_midi_dir, result_list, song_indexes, save_dir=None):
    for i in song_indexes:
        ...
        patterns_est = find_motives(notes, horizontalTolerance=0, verticalTolerance=1
                , adjacentTolerance=(2, 12), min_notes=4, min_cardinality=0.5)

        avg_duration = 0.0
        for j in range(len(patterns_est)):
            avg_duration = avg_duration + (patterns_est[j][0][-1][0] - patterns_est[j][0][0][0])
