...
class CP(object):
    ...
    def extract_events_from_csv(self, csv_note_path, downbeat_csv_path):
        ...
        for i in range(len(dbeats) - 1):
            ...
            for j in range(1, note_stretch):
                dbeats_stretch.append(dbeats[i] * note_stretch + j * (dbeats[i+1] - dbeats[i]))
    ...
    def prepare_data_from_csv(self, csv_note_path, downbeat_csv_path, max_len, offset=0):
        """
            Prepare data for a single midi 
        """
        # extract events
        events, raw_information = self.extract_events_from_csv(csv_note_path, downbeat_csv_path)
        if not events:  # if midi contains nothing
            raise ValueError(f'The given {csv_note_path} is empty')
